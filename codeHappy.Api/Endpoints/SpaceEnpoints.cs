...
public static class SpaceEndpoints
{
    public static void MapSpaceEndpoints(this WebApplication app)
    {
            ...
            CancellationToken ct) =>
        {
            ...
            var space = await service.CreateSpaceAsync(Guid.Parse(userId), request, ct);
        ...
        group.MapGet("/", async (ISpaceService service, ICurrentUserService current, CancellationToken ct) =>
        {
            ...
            var spaces = await service.GetAllSpacesAsync(Guid.Parse(userId), ct);
        ...
        group.MapDelete("/{id}", async (Guid id, ISpaceService service, ICurrentUserService current, CancellationToken ct) =>
        {
            ...
            await service.DeleteSpaceAsync(id, Guid.Parse(userId), ct);

            return Results.NoContent();
        });

        // PUT /spaces/{id} — updates name and icon. Returns 403 if the user is not the owner.
        group.MapPut("/{id}", async (Guid id, UpdateSpaceRequest request, IValidator<UpdateSpaceRequest> validator, ISpaceService service, ICurrentUserService current, CancellationToken ct) =>
        {
            var result = await validator.ValidateAsync(request, ct);
            ...
            await service.UpdateSpaceAsync(id, Guid.Parse(userId), request, ct);

            return Results.NoContent();
        });

        // PATCH /spaces/{id}/touch — updates LastAccessedAt to track recently opened spaces.
        group.MapPatch("/{id}/touch", async (Guid id, ISpaceService service, ICurrentUserService current, CancellationToken ct) =>
        {
            var userId = current.GetUserId();
            ...
            await service.TouchSpaceAsync(id, Guid.Parse(userId), ct);
